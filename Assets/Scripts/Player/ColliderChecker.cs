...
public class ColliderChecker : MonoBehaviour
{
    ...
    [SerializeField] KeyCode interactKey;
    [SerializeField] LayerMask interactuables;
    [SerializeField] GameObject textInteractuable;
    private void Start()
    {
        //textInteractuable.text = "Press " + interactKey + " to open";
    ...
    public void Update()
    {
        ...
        if (colliders.Length > 0) // Verifica si hay colliders en el radio
        {
            ...
            {
                IInteractuable interact = col.GetComponent<IInteractuable>();
                if (interact != null && Input.GetKeyDown(interactKey)) // Verifica si el objeto tiene el script IInteractuable
                {
                    interact.Interact();
                }
            }
